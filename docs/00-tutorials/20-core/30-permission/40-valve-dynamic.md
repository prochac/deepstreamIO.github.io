...
The good news is that deepstream makes realtime permissions extremely easy using a permission language called "Valve". This tutorial assumes that you already know your way around Valve. If you haven't come across it yet, make sure to read the [Simple Valve](valve-simple) and [Advanced Valve](valve-advanced) tutorials first.
...
there is one global color [record](../datasync/records) that can be set by any user to red, green or blue
...
## Login
To keep things simple we'll be using [file-authentication](../auth/file) and cleartext passwords. To enable file-based authentication, configure it in the `auth` section of your server's `config.yml`.
